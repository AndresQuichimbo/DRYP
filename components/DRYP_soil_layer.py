...
class swbm(object):
	...
	def run_soil_aquifer_one_step(self, env_state, sur_elev, wt_elev, Duz0, tht_dt):
		...
		tht = np.where(Duz <= 0.0,env_state.grid.at_node['wilting_point'],tht/Duz)
		self.tht = tht
		self.L_0 = tht*Duz
		self.Duz = Duz
	...
	def run_swb_lat_flow_one_step(self, env_state_soil, env_state_rip):
		# This function estimate lateral flow in river cell, it is assumed that
		# channel width is less than river cell size
...
def SWBMh(I, PET, Kc, L0, SMD_0, z_soil, fs, fc, wp, c, Ksat):
	...
	# Droot	: Soil rooting depth
	# Lsat	: Water content at saturated condition
	d = (c-1)*Ksat/(z_soil*np.power(fs,c))
	PET = Kc*PET
	Pc = 0.5
	Lsat = z_soil*fs
	Lwp = z_soil*wp
	...
	L_aux -= RO
	DL = np.where(L_aux > Lfc, z_soil*np.exp((-c+1)*np.log(np.power(L_aux/z_soil,-c+1)+d))-Lfc,0.0)
